# Written by Dr. Karen Mazidi
# Kiara Madeam HW7
# This program will use a modified bubble sort and its instruction
# count and data cache will be compared with selection sort
# ** USE BUBBLE ALGORITHM FROM CLASS GITHUB **
...
sort:	
	addi	$sp, $sp, -20	# push 5 registers onto stack
	sw	$ra, 16($sp)
	sw	$s3, 12($sp)
	sw	$s2, 8($sp)
	sw	$s1, 4($sp)
...
exit1:	
	lw	$s0, ($sp)	# pop (restore) registers
	lw	$s1, 4($sp)
	lw	$s2, 8($sp)
	lw	$s3, 12($sp)
	lw	$ra, 16($sp)
